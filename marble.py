...
def initialState():
    ...
    print(gt)
    return np.array([10*np.random.random_sample(), 0.0, gt])
...
def plotStatus(net,trial,epsilonTrace,rtrace):
    ...
    plt.subplot(4,3,2)
    plt.plot(X[:,0])
    plt.plot([0,X.shape[0]], [ggo,ggo],'--',alpha=0.5,lw=5)
    plt.ylabel("$x$")
    plt.ylim(-1,11)
    #qs = [[net.use([s,0,a]) for a in actions] for s in range(11)]
    qs = net.use(np.array([[s,0,ggo,a] for a in validActions for s in range(11)]))
    ...
    plt.xlabel("$x$")
    plt.ylabel("$\dot{x}$")
    plt.fill_between([ggo-1,ggo+1],[-5,-5],[ggo,ggo],color="red",alpha=0.3)
    plt.xlim(-1,11)
    plt.ylim(-5,5)
    plt.subplot(4,3,6)
    ...
    xsflat = xs.flat
    ysflat = ys.flat
    qs = net.use(np.array([[xsflat[i],ysflat[i],ggo,a] for a in validActions for i in range(len(xsflat))]))
...
def testIt(Qnet,nTrials,nStepsPerTrial):
    ...
    for x in xs:
        ...
        plt.xlim(-1,11)
        plt.ylim(-5,5)
        plt.plot([ggo,ggo],[-5,5],'--',alpha=0.5,lw=5)
        plt.ylabel('$\dot{x}$')
        plt.xlabel('$x$')
